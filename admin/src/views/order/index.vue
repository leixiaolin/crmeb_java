  ...
  <div class="divBox relative">
    <el-card :bordered="false" shadow="never" class="ivu-mt" :body-style="{ padding: 0 }">
      <div class="padding-add">
        <el-form inline size="small" label-width="75px">
          ...
          <el-form-item label="校园订单：">
            <el-select v-model="tableFrom.campusOnly" placeholder="全部订单" class="selWidth" clearable @change="seachList">
              <el-option label="仅校园配送" :value="true" />
            </el-select>
          </el-form-item>
          <el-form-item label="校园状态：">
            <el-select v-model="tableFrom.campusStatus" placeholder="全部状态" class="selWidth" clearable @change="seachList">
              <el-option
          ...
          <el-form-item label="校园排序：">
            <el-select v-model="tableFrom.campusDeliverySort" placeholder="默认排序" class="selWidth" clearable @change="seachList">
              <el-option label="按配送顺序" :value="true" />
            </el-select>
          </el-form-item>
    ...
    <el-card class="box-card mt14">
      <div slot="header" class="clearfix">
        ...
        <el-button v-hasPermi="['admin:export:excel:order']" @click="exports">导出</el-button>
      </div>
      <el-table
      ...
      >
        ...
        <!-- <el-table-column
          ...
          width="55"
        /> -->
        <el-table-column v-if="checkedCities.includes('订单号')" label="订单号" min-width="210">
        ...
        <el-table-column v-if="checkedCities.includes('订单类型')" prop="orderType" label="订单类型" min-width="110" />
        <el-table-column v-if="checkedCities.includes('收货人')" prop="realName" label="收货人" min-width="100" />
        <el-table-column
...
>{{ val.info.productName + ' | ' }}{{ val.info.sku ? val.info.sku : '-' }}</span>
        ...
        <el-table-column v-if="checkedCities.includes('实际支付')" prop="payPrice" label="实际支付" min-width="80" />
        <el-table-column v-if="checkedCities.includes('支付方式')" label="支付方式" min-width="80">
          <template slot-scope="scope">
            <span>{{ scope.row.payTypeStr }}</span>
          </template>
        </el-table-column>
        <el-table-column v-if="checkedCities.includes('订单状态')" label="订单状态" min-width="100">
          <template slot-scope="scope">
            <div>
              <div v-if="scope.row.refundStatus === 1 || scope.row.refundStatus === 2" class="refunding">
                <template>
                  <el-popover trigger="hover" placement="left" :open-delay="500">
                    <b slot="reference" style="color: #f124c7; cursor: pointer">{{ scope.row.statusStr.value }}</b>
        ...
        <el-table-column v-if="checkedCities.includes('下单时间')" prop="createTime" label="下单时间" min-width="150" />
        <el-table-column label="操作" width="200" fixed="right">
          <template slot-scope="scope">
            <template>
              <a v-if="checkPermi(['admin:order:info'])" @click="onOrderDetails(scope.row.orderId)">详情</a>
            ...
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item
                  v-if="scope.row.campusStatus === 20 && scope.row.refundStatus === 0 && checkPermi(['admin:campus:order:delivery:update'])"
                  @click.native="onCampusDelivered(scope.row)"
                  >校园单标记送达</el-dropdown-item>
                <el-dropdown-item
...
>打印小票</el-dropdown-item>
    ...
    <!--详情-->
    <details-from ref="orderDetail" :order-id="orderId" />

    <!-- 发送货 -->
    <order-send
      ref="send"
      :order-id="orderId"
      :express-list-normal="expressListNormal"
      :express-list-elec="expressListElec"
...
export default {
  ...
  methods: {
    ...
    handleReset() {
      ...
      this.tableFrom.campusOnly = ''
      this.tableFrom.campusStatus = ''
      this.tableFrom.campusDeliverySort = ''
    ...
    onCampusDelivered(row) {
      this.$modalSure('确认校园订单已送达？').then(() => {
        campusOrderDeliveredApi({ orderNo: row.orderId }).then(() => {
          this.$message.success('操作成功')
          this.getList()
        })
      })
